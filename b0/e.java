...
public final class e {
  ...
  private void b() {
    ...
    if (this.i != Double.MAX_VALUE) {
      ...
      if (d > 1.0D) {
        ...
        d1 = -d;
        d2 = this.a;
        this.g = d1 * d2 - d2 * Math.sqrt(d * d - 1.0D);
  ...
  b.o h(double paramDouble1, double paramDouble2, long paramLong) {
    ...
    if (paramDouble1 > 1.0D) {
      paramDouble1 = this.g;
      double d4 = this.f;
      double d3 = d2 - (paramDouble1 * d2 - paramDouble2) / (paramDouble1 - d4);
      paramDouble2 = (d2 * paramDouble1 - paramDouble2) / (paramDouble1 - d4);
      paramDouble1 = Math.pow(Math.E, paramDouble1 * d1) * d3 + Math.pow(Math.E, this.f * d1) * paramDouble2;
      d2 = this.g;
      d4 = Math.pow(Math.E, d2 * d1);
      double d5 = this.f;
      paramDouble2 = d3 * d2 * d4 + paramDouble2 * d5 * Math.pow(Math.E, d5 * d1);
    } else if (paramDouble1 == 1.0D) {
      paramDouble1 = this.a;
      paramDouble2 += paramDouble1 * d2;
      d2 += paramDouble2 * d1;
      paramDouble1 = Math.pow(Math.E, -paramDouble1 * d1) * d2;
      double d3 = Math.pow(Math.E, -this.a * d1);
      double d4 = this.a;
      double d5 = -d4;
      paramDouble2 = paramDouble2 * Math.pow(Math.E, -d4 * d1) + d2 * d3 * d5;
    } else {
      double d4 = 1.0D / this.h;
      double d3 = this.a;
      paramDouble2 = d4 * (paramDouble1 * d3 * d2 + paramDouble2);
      paramDouble1 = Math.pow(Math.E, -paramDouble1 * d3 * d1) * (Math.cos(this.h * d1) * d2 + Math.sin(this.h * d1) * paramDouble2);
      double d5 = this.a;
      d3 = -d5;
      d4 = this.b;
      d5 = Math.pow(Math.E, -d4 * d5 * d1);
      double d7 = this.h;
      double d6 = -d7;
      d7 = Math.sin(d7 * d1);
      double d8 = this.h;
...
/* Location:              C:\Users\Pooya\Desktop\smartpek\Smart-dex2jar.jar!\b0\e.class
